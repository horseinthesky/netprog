...
def fw1(w1, w2, target, data, k, C):
    lenght = len(target)
    S = 0
    for i in range(0, lenght):
        S += target[i] * data[1][i] * (1.0 - 1.0 / (1.0 + math.exp(-target[i] * (w1 * data[1][i] + w2 * data[2][i]))))

    return w1 + (k * (1.0 / lenght) * S) - k * C * w1


def fw2(w1, w2, target, data, k, C):
    lenght = len(target)
    S = 0
    for i in range(0, lenght):
        S += target[i] * data[2][i] * (1.0 - 1.0 / (1.0 + math.exp(-target[i] * (w1 * data[1][i] + w2 * data[2][i]))))
...
def grad(target, data, C=0.0, w1=0.0, w2=0.0, k=0.1, err=1e-5):
    i = 0
    i_max = 10000
    w1_new, w2_new = w1, w2
    ...
    while True:
        i += 1
        w1_new, w2_new = fw1(w1, w2, target, data, k, C), fw2(w1, w2, target, data, k, C)
...
rauc = roc_auc_score(target, y_rscore)
